...
from ..models import SurgeonDevice
from ..email_service import send_email
from ..sms_service import send_sms

router = APIRouter(prefix="/surgeon")

# Separate router mounted at /api/surgeon so the native app can reach OTP
# endpoints at /api/surgeon/otp/request and /api/surgeon/otp/verify.
otp_router = APIRouter()
log = logging.getLogger(__name__)

# ── OTP login for native app ──────────────────────────────────────────────────
# Reuses the MagicLink table: token_hash stores SHA-256 of the 6-digit code.
