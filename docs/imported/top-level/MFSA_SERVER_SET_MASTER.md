...
Env: `/opt/rvu/.env`
SSL: `rvu.midfloridasurgical.com` certificate valid until 2026-06-23 as of 2026-05-18.
...
- Largest tables:
  - `rvu_scans=655`
  - `rvu_scan_ai_runs=219`
  - `rvu_staff_devices=13`
  - `rvu_staff=8`
  - `rvu_op_notes=7`

Fresh backup uploaded:

```text
wasabi:mfsa-cal/rvu-backups/20260518-135317
```
...
- GitHub deploy key was added on `.61` for `nailecompsys-coder/RVU.git`.
- GitHub remote `github` was added on `.61`: `git@github.com-RVU-deploy:nailecompsys-coder/RVU.git`.
- GitHub `main` was initialized from committed RVU baseline `6516338` on 2026-05-18.
- RVU `origin` still points to Gitea while the production worktree is dirty; do not switch tracking/remotes until the uncommitted live changes are reviewed and either committed or intentionally discarded.
